    ...
    {   if (!GameMaster.instance.paused && GameMaster.instance.whiteIn == false)
        {
            ...
            direction = -direction;
            if (Mathf.Abs(direction.x) < 5f && Mathf.Abs(direction.y) < 5f && rb.velocity.magnitude == 0 && mouseBeingHeld)
            {
            ...
            if (Input.GetMouseButtonUp(0) && Mathf.Abs(direction.x) < 5f && Mathf.Abs(direction.y) < 5f && rb.velocity.magnitude == 0 && !GameMaster.instance.areMoving && mouseBeingHeld)
            {
                isHit = true;
